...
const index = () => {
  ...
  const handleMessaged = async (phone) => {
    try {
          ...
          renderItem={({ item }) =>
            ...
            >
              <View style={{ borderBottomWidth: 1, alignItems: 'center', borderBottomColor: 'grey', height: 80, width: 400, flexDirection: 'row', gap: 15 }}>
                ...
                <View style={{ gap: 10 }}>
                  <Text style={{ fontSize: 20, fontStyle: 'bold', alignItems: 'flex-start' }}>{item.name}</Text>
                  <Text style={{ fontSize: 10, color: 'grey', alignItems: 'flex-start' }}>Last message</Text>
                </View>
              </View>
            </Pressable>
          }
